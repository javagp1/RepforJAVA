     ...
     <div   style="background-color: rgb(0,255,64);width: 150px;margin: 10px auto;font-size: 15px;border-radius: 8px;padding: 0 10px;height: 50px;line-height: 50px">商品信息管理<a title="添加信息" class="btn glyphicon glyphicon-plus" href="/springMVC/goodsinfoctrl/editinsert"></a></div>
     	...
     	<table class="table table-hover text-center table-striped">
     		...
     		<tbody>
     			...
     			<x:forEach items="${requestScope.goods}" var="ai">
     				<tr>
     					<td>${ai.gdid}</td>
     					<td  style="text-align: right">${ai.gdname}<a class="btn btn-link" href="/springMVC/goodsinfoctrl/editupdate?gdid=${ai.gdid}&pagenum=${requestScope.currentpagenum}" title="修改信息"><i class="glyphicon glyphicon-cog"></i> </a></td>
     					<td>${ai.stid}</td>
     					<td>${ai.gtkeywords}</td>
     					<td>${ai.gtdate}</td>
     					<td><button btn_self="delete" gdid="${ai.gdid}" class="btn btn-danger">delete</button></td>
     				</tr>
     				</x:forEach>
     	...
     	<div class="text-center">
     	 <a class="btn " href="/springMVC/goodsinfoctrl/getall?pagenum=1">首页</a>
     	 <x:set var="previous" value="${requestScope.currentpagenum-1}" scope="request"></x:set>
     	 <x:if test="${requestScope.currentpagenum>1}">
     	 	<a class="btn " href="/springMVC/goodsinfoctrl/getall?pagenum=${previous}">上一页</a>
     	 </x:if>
     	<x:forEach begin="1" end="${requestScope.pagecount}" var="i" step="1">
     		...
     		<x:if test="${not eqi}">
     			<a class="btn " href="/springMVC/goodsinfoctrl/getall?pagenum=${i}">${i}</a>
     		</x:if>
     	</x:forEach>
     		 ...
     		 <x:set var="next" value="${requestScope.currentpagenum+1}" scope="request"></x:set>
     		  <x:if test="${requestScope.currentpagenum<requestScope.pagecount}">
     	 		<a class="btn " href="/springMVC/goodsinfoctrl/getall?pagenum=${next}">下一页</a>
     		</x:if>
     		
   			 <a class="btn " href="/springMVC/goodsinfoctrl/getall?pagenum=${requestScope.pagecount }">尾页</a>
     	</div>
     </div>
  </body>
  ...
  $("").ready(function(){
  	...
  	$("[btn_self=delete]").click(function(){
  		...
  		if(window.confirm("确定要删除吗？")){
  			window.location.href="/springMVC/goodsinfoctrl/dodelete?pagenum=${requestScope.currentpagenum}&gdid="+$(this).attr("gdid");
  		}
